...
as $$
  with raw_search as (
    select left(btrim(coalesce(p_search_text, '')), 500) as raw
  ), normalized_search as (
    select case
      when raw ~* '^[[:space:]]*(what|where|when|who|how)[[:space:]]+(did|does|do)[[:space:]]+'
      then btrim(regexp_replace(
        regexp_replace(
          regexp_replace(
            regexp_replace(raw, '^[[:space:]]*(what|where|when|who|how)[[:space:]]+(did|does|do)[[:space:]]+', '', 'i'),
            '[[:space:]]+(say|said|tell|told)[[:space:]]+(about[[:space:]]+)?',
            ' ',
            'i'
          ),
          '[[:space:]]+(the|a|an)[[:space:]]+',
          ' ',
          'gi'
        ),
        '[?]+$',
    ...
    select
      websearch_to_tsquery('english'::regconfig, normalized) as query,
      left(trim(both '"' from normalized), 500) as literal
...
comment on function public.search_testimony(uuid, text, integer, integer, integer) is
  'RLS-aware testimony search that normalizes common questions before combining canonical-text FTS and trigram retrieval with provenance and context.';
