...
import { courtRecordHref, parseStructureObjectType, structureHref, structureObjectTypes, structureReviewHref } from "@/lib/case-routes";
...
export default async function StructurePage({ params, searchParams }: { params: Promise<{ caseId: string }>; searchParams: SearchParams }) {
  ...
  return <main className="structure-shell">
    <PageHeader eyebrow="STRUCTURE · READ-ONLY LINEAGE WORKSPACE" title="Objects with their receipts." lede="Browse candidate, reviewed, and canonical objects without promoting, merging, or mutating evidence." actions={<Link className="ds-btn" href={structureReviewHref(caseId, { reviewStatus: "pending" })}>Open governed review queue →</Link>} aside={<div className="structure-coverage"><strong>{workspace.coverage.linkedSegments.toLocaleString()} <span>/ {workspace.coverage.totalSegments.toLocaleString()}</span></strong><p>RLS-visible source segments linked to at least one structural object.</p><div><i style={{ width: `${Math.min(linkedPercent, 100)}%` }} /></div><small>{linkedPercent.toFixed(1)}% case-scoped lineage coverage</small></div>} />
