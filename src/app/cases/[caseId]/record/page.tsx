...
import { TranscriptViewSelect } from "@/app/cases/[caseId]/record/view-select";
...
export default async function CourtRecordPage({ params, searchParams }: { params: Promise<{ caseId: string }>; searchParams: SearchParams }) {
  ...
  return <main className="court-record-shell">
    ...
    {workspace.totalSegments === 0 ? <section className="empty-court-record"><MonoLabel>NO COMMITTED CORPUS</MonoLabel><h2>The testimony database is not ready.</h2><p>No committed transcript is available for this case. Supporting pictures remain separate from the canonical record.</p><Link href={caseFilesHref(caseId)}>Open supporting Files →</Link></section> : <div className="court-record-grid">
      ...
      <section className="court-transcript-panel" aria-label="Windowed testimony database"><header><div><MonoLabel>CANONICAL TRANSCRIPT WINDOW</MonoLabel><h2>{workspace.proceeding?.title ?? workspace.artifact?.title ?? "Source record"}</h2><TranscriptViewSelect caseId={caseId} query={query} segmentId={selected?.id} proceedingId={queryState.segment ? undefined : queryState.proceeding} view={textView ? "text" : "segments"} /></div><span className="court-window-count">{workspace.segments.length} segments loaded</span></header>{selected ? (textView ? <TranscriptText title={workspace.proceeding?.title ?? workspace.artifact?.title ?? "Testimony"} segments={workspace.segments} selectedId={selected.id} /> : <TranscriptWindow caseId={caseId} query={query} selectedId={selected.id} segments={workspace.segments} />) : null}</section>
