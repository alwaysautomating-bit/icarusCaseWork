...
type SearchParams = Promise<{ day?: string; q?: string; notice?: string }>;
...
export default async function TrialIndexPage({ params, searchParams }: { params: Promise<{ caseId: string }>; searchParams: SearchParams }) {
  ...
  return <main className="trial-index-shell">
    ...
    <form className="trial-index-search" method="get"><label><span>Find a witness, topic, or day summary</span><input name="q" defaultValue={query} maxLength={300} placeholder="Apple Watch, 911 call, Stratton, defense begins…" /></label><button>Search index</button></form>
    <div className="trial-index-workspace">
      <aside className="trial-index-days"><header><MonoLabel>TRIAL CALENDAR</MonoLabel><strong>{workspace.visibleDays.length}</strong></header>{workspace.visibleDays.length === 0 ? <p>No indexed day matches this lookup.</p> : workspace.visibleDays.map((day) => <Link className={selected?.id === day.id ? "selected" : ""} href={trialIndexHref(caseId, { dayNumber: day.day_number, query })} key={day.id}><span>DAY {day.day_number}</span><strong>{day.headline}</strong><small>{displayDate(day.court_date)} · {day.trial_phase.replaceAll("_", " ")}</small><em>{day.proceeding_id ? "CANONICAL PROCEEDING" : day.basis.replaceAll("_", " ")}</em></Link>)}</aside>
      ...
      <aside className="trial-index-editor">{workspace.canManage ? <TrialIndexForm caseId={caseId} day={selected} nextDayNumber={nextDayNumber} proceedings={workspace.proceedings} key={selected?.id ?? "new"} /> : <div className="trial-index-readonly"><MonoLabel>READ-ONLY MEMBERSHIP</MonoLabel><p>Owners and reviewers manage the index. You can use every day, witness, topic, and canonical link for navigation.</p></div>}</aside>
    </div>
  </main>;
}
