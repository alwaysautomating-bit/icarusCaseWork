...
export default async function QuestionsPage({ params, searchParams }: { params: Promise<{ caseId: string }>; searchParams: Promise<SearchState> }) {
  ...
  return <main className="research-queue-shell">
    <header className="research-page-heading"><div><MonoLabel>WORKING RESEARCH QUEUE</MonoLabel><h1>Questions</h1><p>A question should survive longer than the moment that caused someone to ask it.</p></div><aside><strong>{workspace.questions.filter((item) => item.status === "open").length}</strong><span>OPEN QUESTIONS</span></aside></header>
    ...
    <form method="get" className="research-search-bar"><label><span>Search questions</span><input name="q" defaultValue={query.q ?? ""} placeholder="hand swabs, doorway, testing…" /></label><label><span>Status</span><select name="status" defaultValue={status}><option value="open">Open</option><option value="resolved">Resolved</option><option value="all">All</option></select></label><button>Search</button>{canContribute && <Link href={`${questionsHref(caseId)}?ask=1`}>+ Ask question</Link>}</form>
